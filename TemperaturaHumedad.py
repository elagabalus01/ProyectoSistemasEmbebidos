...
class TemperaturaHumedad():
  ...
  def __init__(self,bot,app):
    ...
    def temperatura(message):
      bot.send_message(Constantes.chat_id(),"La temperatura es: "+str(math.trunc(self.tmpTemperatura))+"°C")

    # Función de comando para mostrar humedad	
    @bot.message_handler(commands=["humedad"])
    def humedad(message):
      bot.send_message(Constantes.chat_id(),"La humedad es: "+str(math.trunc(self.tmpHumedad))+"%")
